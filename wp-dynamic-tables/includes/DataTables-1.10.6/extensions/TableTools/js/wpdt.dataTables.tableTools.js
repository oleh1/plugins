...
TableTools.BUTTONS = {
	...
	"copy": $.extend( {}, TableTools.buttonBase, {
		...
		"fnComplete": function(nButton, oConfig, flash, text) {
			...
			display = oConfig.sInfo;

			if( lines === 1 ){

				display = display.replace( "[rows_num]", lines + " " + oConfig.sLine );
			}
			else{

				display = display.replace( "[rows_num]", lines + " " + oConfig.sLines );
			}

			this.fnInfo( display, 1500 );
		}
	} ),
